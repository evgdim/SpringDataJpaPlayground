...
public class CustomerRepositoryImpl implements CustomerRepositoryCustom {
	...
	@Override
	@Transactional
	public int deleteByCategoryIdSession(long categoryId) {
		Session session = em.unwrap(Session.class);
		org.hibernate.Query deleteQuery = session.createQuery("delete from Customer c where c.category.id = :categoryId");
		deleteQuery.setParameter("categoryId", categoryId);
		int delCnt = deleteQuery.executeUpdate();
		return delCnt;
	}
